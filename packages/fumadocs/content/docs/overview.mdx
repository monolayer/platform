---
title: Overview
Order: 1
---

The `@monolayer/sdk` includes the `monolayer` command-line interface (CLI), which helps you manage your projects and deployments seamlessly.

## Available commands

The Monolayer CLI currently supports the following commands:

| Command | Description | Output |
| --- | --- | --- |
| `projects:list` | Lists your projects using cursor-based pagination. | JSON |
| `deployments:deploy` | Triggers a branch deployment and monitors its progress. | Logs and status |

## Quick reference

To get started quickly, you can run these commands directly from your terminal.

**List your projects**

```bash
MONOLAYER_BASE_URL="https://control-plane-domain" \
MONOLAYER_AUTH_TOKEN="token_xxx" \
npx monolayer projects:list --limit 5
```

**Trigger a deployment**

```bash
MONOLAYER_BASE_URL="https://control-plane-domain" \
MONOLAYER_DEPLOYMENT_TOKEN="deploy_token_xxx" \
npx monolayer deployments:deploy --project-id proj-1 --branch-name main
```

## Learn more

Explore the following topics to get the most out of the Monolayer CLI:

- **[Installation](./installation):** Set up the SDK in your environment.
- **[Authentication and Configuration](./authentication):** Securely connect to your control plane.
- **[CLI Reference](./cli):** Detailed guides for each command.
