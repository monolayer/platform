...
import BranchTrackingSet from "../../src/commands/projects/branch-tracking/set.js";
...
describe("projects:branch-tracking:set command", () => {
	afterEach(() => {
		vi.unstubAllGlobals();
	...
	it("prints JSON response on successful set with --enabled", async () => {
	...
	it("prints JSON response on successful set with --no-enabled", async () => {
		const fetchMock = vi.fn().mockResolvedValue(
			jsonResponse(200, { success: true }),
		);
		vi.stubGlobal("fetch", fetchMock as unknown as typeof fetch);
