...
export default class EnvDelete extends BaseCommand {
	static summary = "Delete an environment variable";
	static description = "Deletes an environment variable from the project.";

	static examples = [
		"<%= config.bin %> <%= command.id %> --base-url https://control-plane-domain --auth-token deploy_token_... --project-id proj-1 --name API_KEY --environment production",
	];
	...
	static flags = {
		...
		name: Flags.string({
			required: true,
			summary: "Environment variable name/key to delete",
		}),
	...
	public async run(): Promise<{ readonly success: true }> {
		...
		const result = await client.environmentVariables.deletePromise({
