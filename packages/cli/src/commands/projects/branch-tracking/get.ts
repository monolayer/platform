...
import type { BranchTrackingDto } from "../../../client/types.js";

export default class BranchTrackingGet extends BaseCommand {
	static summary = "Get branch tracking settings";
	static description = "Returns branch tracking configuration for a project.";
	...
	public async run(): Promise<BranchTrackingDto> {
		const { flags } = await this.parse(BranchTrackingGet);
		...
		const result = await client.branchTracking.getPromise({
